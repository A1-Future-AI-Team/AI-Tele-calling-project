    ...
    async transcribeAudio(req, res) {
        try {
            ...
            try {
                ...
                // GUARD: skip saving if transcription is invalid or very short
                let wordCount = transcribedText.trim().split(/\s+/).filter(Boolean).length;
                if (!transcribedText || transcribedText.trim().toLowerCase() === 'no transcription available' || wordCount === 0) {
                    ...
                    let repeatMsg = currentLanguage === 'Bengali' ? 'দয়া করে আবার বলুন।' : currentLanguage === 'Hindi' ? 'कृपया फिर से कहें।' : 'Sorry, I did not catch that. Please repeat.';
                    if (failCount < 3) {
                        twiml.say({ voice: 'alice', language: this.mapLanguageToTwimlLanguage(currentLanguage) }, repeatMsg);
                        twiml.record({
                            action: `/api/twilio/transcribe?campaignId=${campaignId}`,
                            method: 'POST',
                            maxLength: 10,
                        ...
                        let endMsg = currentLanguage === 'Bengali' ? 'ধন্যবাদ! কল শেষ করা হচ্ছে।' : currentLanguage === 'Hindi' ? 'धन्यवाद! कॉल समाप्त की जा रही है।' : 'Thank you! Ending the call.';
                        twiml.say({ voice: 'alice', language: this.mapLanguageToTwimlLanguage(currentLanguage) }, endMsg);
                        res.type('text/xml');
